...
class TaskRunner:
    ...
    def run(self, config):
        ...
        # initialize tokenizer and processor for OmniGen2 since tokenizer and processor are inside pipeline initialized in worker
        ...
        if config.reward_model.enable:
            if config.reward_model.strategy == 'fsdp':
                ...
                from verl.workers.fsdp_workers import create_reward_model_worker
                RewardModelWorker = create_reward_model_worker(config.reward_model)
        ...
        elif reward_manager_name == 'image_generation':
            from verl.workers.reward_manager import ImageGenerationRewardManager
            reward_manager_cls = ImageGenerationRewardManager
        else:
            raise NotImplementedError
        ...
        val_reward_fn = reward_manager_cls(tokenizer=tokenizer, num_examine=1, compute_score=compute_score, img_saving_args=config.img_saving, eval=True)
