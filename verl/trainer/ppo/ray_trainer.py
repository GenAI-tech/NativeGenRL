...
from verl.utils.dataset.rl_dataset import RLHFDataset, collate_fn, DummyJanusDPORLHFDataset, JanusTextOnlyRLHFDataset, OmniTextOnlyRLHFDataset
...
def apply_kl_penalty(data: DataProto, kl_ctrl: core_algos.AdaptiveKLController, kl_penalty='kl'):
    ...
    response_mask = torch.ones_like(data.batch['old_log_probs']) # no response mask for image generation; (batch, # timesteps)
...
class RayPPOTrainer(object):
    ...
    def _create_dataloader(self):
        ...
        # self.train_dataset = JanusTextOnlyRLHFDataset(parquet_files=self.config.data.train_files,
        self.train_dataset = OmniTextOnlyRLHFDataset(parquet_files=self.config.data.train_files,
        ...
        self.val_dataset = OmniTextOnlyRLHFDataset(parquet_files=self.config.data.val_files,
                                       tokenizer=self.tokenizer,
                                       processor=self.processor,
                                       prompt_key=self.config.data.prompt_key,
                                       image_key=self.config.data.get('image_key', 'images'),
        ...
        for epoch in range(self.config.trainer.total_epochs):
            ...
            for batch_dict in self.train_dataloader:
                ...
                with _timer('step', timing_raw):
                    ...
                    print('repeat batch', batch.batch)
                    ...
                    # for image diffusion, batch already has log_probs; no, KL is between old_log_probs (needs grads) and ref_log_probs
                    # if 'log_probs' in batch.batch.keys():
                    #     # assign log_probs to old_log_prob
                    #     old_log_prob = batch.batch['log_probs']
                    #     old_log_prob_batch = DataProto.from_dict(tensors={'old_log_probs': old_log_prob},
                    #                                              meta_info={'temperature': 1.0})
                    #     batch = batch.union(old_log_prob_batch)
                    # else:
                    #     with _timer('old_log_prob', timing_raw):
                    #         old_log_prob = self.actor_rollout_wg.compute_log_prob(batch)
                    #         batch = batch.union(old_log_prob)  # (batch_size, num_inference_steps)
                    
                    # print('old_log_prob', old_log_prob.batch['old_log_probs'])
                    ...
                    # print('ref_log_prob', ref_log_prob.batch['ref_log_prob'])
                ...
                # collect metrics
                metrics.update(compute_data_metrics(batch=batch, use_critic=self.use_critic, use_mask=False))  # no response mask for diffusion image generation
