...
class DataProto:
    ...
    def concat(data: List['DataProto']) -> 'DataProto':
        ...
        raw_non_tensor_list = [d.non_tensor_batch for d in data]
        merged_non_tensor = list_of_dict_to_dict_of_list(raw_non_tensor_list)

        for key, val in merged_non_tensor.items():
            try:
                # Try normal concatenation (for fixed-shape arrays)
                merged_non_tensor[key] = np.concatenate(val, axis=0)
            except ValueError:
                # Fallback: flatten and store as object dtype (for variable-length entries)
                merged_non_tensor[key] = np.array(
                    [item for sublist in val for item in sublist], dtype=object
                )

        return DataProto(batch=new_batch, non_tensor_batch=merged_non_tensor, meta_info=data[0].meta_info)
