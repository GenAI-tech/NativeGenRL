...
class FMPipelineOutput(BaseOutput):
    ...
    Args:
        ...
        log_probs (Optional[List[torch.Tensor]]): 
            List of log probabilities for each denoising step. Each tensor has shape (batch_size,).
        diffusion_data (Optional[Dict]): 
            Dictionary containing intermediate diffusion states (latents, timesteps, embeddings) for training.
    """
    images: Union[List[PIL.Image.Image], np.ndarray]
...
class OmniGen2Pipeline(DiffusionPipeline, OmniGen2LoraLoaderMixin):
    ...
    def disable_cfg(self):
        self.cfg_enabled = False
        print(f"CFG disabled for OmniGen2Pipeline")
    ...
    def __call__(
        ...
        verbose: bool = False,
        step_func=None,
        noise_level: float = 0.3,  # 0.7 is too large to make vague images
    ):
        ...
        image, log_probs, diffusion_data = self.processing(
            latents=latents,
        ...
        print(f"call log_probs: {log_probs}")
        ...
        if not return_dict:
            return image, log_probs
        else:
            return FMPipelineOutput(images=image, log_probs=log_probs, diffusion_data=diffusion_data)
    ...
    def processing(
        ...
        timesteps, num_inference_steps = retrieve_timesteps(
            ...
            device,
            timesteps,
            num_tokens=latents.shape[-2] * latents.shape[-1]  # has to be omnigen2 local scheduler in scheduling_flow_match_euler_discrete.py
        )
        num_warmup_steps = max(len(timesteps) - num_inference_steps * self.scheduler.order, 0)
        self._num_timesteps = len(timesteps)

        # Initialize lists to collect log probabilities and intermediate states
        all_log_probs = []
        all_latents = []  # Store latents at each step
        all_next_latents = []  # Store next latents at each step
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                # Store current latents and timestep
                current_latents = latents.clone()
                current_timestep = t.unsqueeze(0).expand(batch_size)
                ...
                if not self.cfg_enabled: # no CFG
                ...
                # Store next latents after the step
                all_next_latents.append(latents.clone())
                
                # Collect log probabilities
                all_log_probs.append(log_prob)  # (batch_size, )
        ...
        all_log_probs = torch.stack(all_log_probs, dim=1)  # (batch_size, num_inference_steps)
        all_latents = torch.stack(all_latents, dim=1)  # (batch_size, num_steps, ...)
        all_next_latents = torch.stack(all_next_latents, dim=1)  # (batch_size, num_steps, ...)
        all_timesteps = torch.stack(all_timesteps, dim=1)  # (batch_size, num_steps)
        
        # Create diffusion data dictionary with all necessary information for training
        diffusion_data = {
            'latents': all_latents,
        ...
        return image, all_log_probs, diffusion_data
    ...
    def compute_log_prob(
        ...
        """
        Compute log probabilities for given latents and timesteps with batched optimization.
        
        This method is called externally for recomputing log probabilities during training.
        
        OPTIMIZATION: Instead of processing each timestep individually, we group timesteps
        that have the same CFG behavior and process them in batches. This reduces the number
        of forward passes through the transformer model, significantly improving performance.
        
        For example, if timesteps 0-10 have CFG off and timesteps 11-20 have CFG on,
        we process each group in 2 batched forward passes instead of 21 individual passes.
        
        Args:
            latents: (batch_size, num_steps, ...) - latents at each timestep
            next_latents: (batch_size, num_steps, ...) - next latents after each step
            timesteps: (batch_size, num_steps) - timesteps
            ...
            negative_prompt_attention_mask: negative prompt attention mask
            ref_latents: reference image latents
            noise_level: noise level for SDE step
            cfg_range: tuple defining the range where CFG is active
            text_guidance_scale: scale for text guidance
            image_guidance_scale: scale for image guidance
            num_inference_steps: total number of inference steps
            
        Returns:
            log_probs: (batch_size, num_steps) - log probabilities for each timestep
        """
        device = self._execution_device

        if not hasattr(self.scheduler, '_timesteps'):  # _timesteps is set in __call__ but not here
        ...
        # Use same cache mechanism as processing() if available
        enable_taylorseer = getattr(self, "enable_taylorseer", False)
        if enable_taylorseer:
            model_pred_cache_dic, model_pred_current = cache_init(self, num_steps)
            model_pred_ref_cache_dic, model_pred_ref_current = cache_init(self, num_steps)
            model_pred_uncond_cache_dic, model_pred_uncond_current = cache_init(self, num_steps)
            self.transformer.enable_taylorseer = True
        elif self.transformer.enable_teacache:
            # Use different TeaCacheParams for different conditions
            teacache_params = TeaCacheParams()
        ...
        with torch.autocast(device_type='cuda', dtype=torch.bfloat16):
            # Recompute log probabilities for each timestep
            for j in range(num_steps):
                # Get current latents and timestep
                current_latents = latents[:, j]  # (batch_size, ...)
                current_next_latents = next_latents[:, j]  # (batch_size, ...)
                current_timestep = timesteps[:, j]  # (batch_size,)
                
                # Use same CFG logic as processing()
                text_guidance_scale = text_guidance_scale if cfg_range[0] <= j / num_steps <= cfg_range[1] else 1.0
                image_guidance_scale = image_guidance_scale if cfg_range[0] <= j / num_steps <= cfg_range[1] else 1.0
                
                if not self.cfg_enabled: # no CFG
                    text_guidance_scale = 1.0
                    image_guidance_scale = 1.0

                # Set cache parameters if using teacache
                if enable_taylorseer:
                    self.transformer.cache_dic = model_pred_cache_dic
                    self.transformer.current = model_pred_current
                elif self.transformer.enable_teacache:
                    teacache_params.is_first_or_last_step = j == 0 or j == num_steps - 1
                    self.transformer.teacache_params = teacache_params

                # Predict using the same method as processing()
                model_pred = self.predict(
                    t=current_timestep[0],  # predict expects single timestep
                ...
                # Apply same CFG logic as processing()
                if text_guidance_scale > 1.0 and image_guidance_scale > 1.0:
                    if enable_taylorseer:
                        self.transformer.cache_dic = model_pred_ref_cache_dic
                        self.transformer.current = model_pred_ref_current
                    elif self.transformer.enable_teacache:
                        teacache_params_ref.is_first_or_last_step = j == 0 or j == num_steps - 1
                        self.transformer.teacache_params = teacache_params_ref
                    ...
                    if enable_taylorseer:
                        ...
                        teacache_params_uncond.is_first_or_last_step = j == 0 or j == num_steps - 1
                        self.transformer.teacache_params = teacache_params_uncond
        ...
        # Clean up cache if using taylorseer
        if enable_taylorseer:
            del model_pred_cache_dic, model_pred_ref_cache_dic, model_pred_uncond_cache_dic
            del model_pred_current, model_pred_ref_current, model_pred_uncond_current
        # Stack log probabilities: (batch_size, num_steps)
